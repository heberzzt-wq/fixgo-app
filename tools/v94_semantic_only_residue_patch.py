...
if count == 1:
    ...
    raise SystemExit(f'RESIDUE_ANCHOR_FAILED:terminal-critical-filter-a:{count}')

old_filter_b = '''file.critical === true ||\n            /jarvis|runtime|engine|terminal|bridge|executor|planner|repo/i.test(\n                `${file.file || ""} ${file.module || ""} ${file.type || ""}`\n            )'''
count = text.count(old_filter_b)
if count == 1:
    text = text.replace(old_filter_b, 'file.critical === true', 1)
elif count > 1:
    raise SystemExit(f'RESIDUE_ANCHOR_FAILED:terminal-critical-filter-b:{count}')
write(path, text)
