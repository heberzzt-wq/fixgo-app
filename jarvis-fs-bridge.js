...
function runHuMoLanPowerShell(authority, script, { execFileSyncImpl = execFileSync, timeoutMs = 120000 } = {}) {
    ...
    ), { encoding: "utf8", windowsHide: true, timeout: timeoutMs, maxBuffer: 4 * 1024 * 1024, input: String(script) }) || "").trim();
}
...
export function inspectNextIdentityRuntimeCandidate({ backend = "humo-17b-identity" } = {}) {
    ...
    return {
        ...
        assetPlacementPlan: strategy ? {
            ...
            persistentTotalWithExistingCacheBytes: strategy.auxiliaryAssetsPersistent === true ? Number(strategy.requiredBytesKeepingExistingCache || 0) : Number(strategy.persistentCoreBytes || 0),
            ephemeralAssets: strategy.auxiliaryAssetsPersistent === true ? [] : (strategy.wrapperAuxiliaryAssets || []).map(asset => ({ ...asset })),
            ephemeralAssetsBytes: strategy.auxiliaryAssetsPersistent === true ? 0 : Number(strategy.wrapperAuxiliaryBytes || 0),
...
export function buildHuMo17RuntimeProbeJob({ assets, hardBudgetUsd, operationId, paidAuthorized = false } = {}) {
    ...
    return {
        ...
        effectiveRuntimeConfig: {
            ...
            model: "HuMo-17B",
            comfyUiRevision: candidate.singleGpuStrategy.comfyUiRevision,
            wrapperRevision: candidate.singleGpuStrategy.wrapperRevision,
            videoTransformer: path.posix.basename(candidate.singleGpuStrategy.quantizedModel.path),
            distillationLora: path.posix.basename(candidate.singleGpuStrategy.distillationLora.path),
            distillationLoraStrength: 1.0,
            ...
            compileEnabled: candidate.singleGpuStrategy.compileEnabled,
            modelLoaderDevice: candidate.singleGpuStrategy.modelLoaderDevice,
            blockSwapWorkflowWidgetValues: candidate.singleGpuStrategy.blockSwapWorkflowWidgetValues
        },
...
export function buildHuMo17AuxiliaryDownloader() {
    return [
        "import json, pathlib, hashlib, urllib.request, urllib.error, socket, time, os, re, http.client",
        "",
        "def aux_marker(status, a, attempt, existing, downloaded, **extra):",
        "    print(json.dumps(dict(status=status, role=a['role'], path=a['path'], attempt=attempt,",
        "        existingBytes=existing, downloadedBytes=downloaded, expectedBytes=a['bytes'], **extra)), flush=True)",
        "",
        "def aux_verify(p, a):",
        "    if not p.is_file() or p.stat().st_size != a['bytes']:",
        "        raise ValueError('HUMO17_AUX_SIZE_INVALID')",
        "    h = hashlib.sha256()",
        "    with p.open('rb') as f:",
        "        for b in iter(lambda: f.read(8*1024*1024), b''):",
        "            h.update(b)",
        "    if h.hexdigest() != a['sha256']:",
        "        raise ValueError('HUMO17_AUX_SHA_INVALID')",
        "",
        "def download_auxiliary(a, p, url, attempts=6, connect_timeout=30, read_timeout=180, backoff=2):",
        "    if not isinstance(a.get('bytes'), int) or a['bytes'] <= 0 or not re.fullmatch(r'[a-f0-9]{64}', a.get('sha256', '')):",
        "        raise ValueError('HUMO17_AUX_CONTRACT_INVALID')",
        "    p = pathlib.Path(p)",
        "    partial = p.with_suffix('.partial')",
        "    if p.is_symlink() or partial.is_symlink():",
        "        raise ValueError('HUMO17_AUX_SYMLINK')",
        "    if p.exists():",
        "        aux_verify(p, a)",
        "        aux_marker('HUMO17_AUX_SHA_VERIFIED', a, 0, a['bytes'], a['bytes'])",
        "        return",
        "    p.parent.mkdir(parents=True, exist_ok=True)",
        "    for attempt in range(1, attempts+1):",
        "        existing = partial.stat().st_size if partial.exists() else 0",
        "        if existing > a['bytes']:",
        "            raise ValueError('HUMO17_AUX_PARTIAL_OVERSIZE')",
        "        if existing == a['bytes']:",
        "            aux_verify(partial, a)",
        "            aux_marker('HUMO17_AUX_SHA_VERIFIED', a, attempt, existing, existing)",
        "            partial.rename(p)",
        "            aux_marker('HUMO17_AUX_DOWNLOAD_COMPLETED', a, attempt, existing, existing)",
        "            return",
        "        aux_marker('HUMO17_AUX_DOWNLOAD_STARTED', a, attempt, existing, existing)",
        "        headers = {'Accept-Encoding': 'identity'}",
        "        if existing:",
        "            headers['Range'] = 'bytes='+str(existing)+'-'",
        "        try:",
        "            with urllib.request.urlopen(urllib.request.Request(url, headers=headers), timeout=connect_timeout) as src:",
        "                src.fp.raw._sock.settimeout(read_timeout)",
        "                if src.headers.get('Content-Encoding', 'identity') != 'identity':",
        "                    raise ValueError('HUMO17_AUX_ENCODING_INVALID')",
        "                status = src.status",
        "                mode = 'wb'",
        "                expected_response = a['bytes']",
        "                if status == 206:",
        "                    match = re.fullmatch(r'bytes (\\d+)-(\\d+)/(\\d+)', src.headers.get('Content-Range', ''))",
        "                    if not match:",
        "                        raise ValueError('HUMO17_AUX_CONTENT_RANGE_INVALID')",
        "                    start, end, total = map(int, match.groups())",
        "                    if start != existing or total != a['bytes'] or end < start or end >= total:",
        "                        raise ValueError('HUMO17_AUX_CONTENT_RANGE_INVALID')",
        "                    expected_response = end-start+1",
        "                    mode = 'ab'",
        "                    aux_marker('HUMO17_AUX_DOWNLOAD_RESUMED', a, attempt, existing, existing)",
        "                elif status == 200:",
        "                    if existing:",
        "                        aux_marker('HUMO17_AUX_RANGE_IGNORED_RESTART', a, attempt, existing, 0)",
        "                else:",
        "                    raise ValueError('HUMO17_AUX_HTTP_STATUS_INVALID')",
        "                length = src.headers.get('Content-Length')",
        "                if length is not None and (not length.isdigit() or int(length) != expected_response):",
        "                    raise ValueError('HUMO17_AUX_CONTENT_LENGTH_INVALID')",
        ...
        "                    try:",
        "                        while True:",
        "                            b = src.read1(min(1024*1024, a['bytes']-downloaded+1))",
        "                            if not b:",
        "                                break",
        "                            if downloaded+len(b) > a['bytes'] or response_bytes+len(b) > expected_response:",
        "                                raise ValueError('HUMO17_AUX_RESPONSE_OVERSIZE')",
        "                            dst.write(b)",
        "                            downloaded += len(b)",
        "                            response_bytes += len(b)",
        "                            if downloaded-progress_at >= 64*1024*1024:",
        "                                dst.flush()",
        "                                aux_marker('HUMO17_AUX_DOWNLOAD_PROGRESS', a, attempt, existing, downloaded)",
        "                                progress_at = downloaded",
        "                    finally:",
        "                        dst.flush()",
        "                        os.fsync(dst.fileno())",
        "                if downloaded != a['bytes'] or response_bytes != expected_response:",
        "                    raise ConnectionError('HUMO17_AUX_TRUNCATED')",
        "            aux_verify(partial, a)",
        "            aux_marker('HUMO17_AUX_SHA_VERIFIED', a, attempt, existing, downloaded)",
        "            partial.rename(p)",
        "            aux_marker('HUMO17_AUX_DOWNLOAD_COMPLETED', a, attempt, existing, downloaded)",
        "            return",
        "        except (TimeoutError, socket.timeout, urllib.error.URLError, ConnectionError, http.client.IncompleteRead, http.client.RemoteDisconnected) as error:",
        "            if isinstance(error, urllib.error.HTTPError) and error.code not in (408,429,500,502,503,504):",
        "                raise RuntimeError('HUMO17_AUX_HTTP_'+str(error.code)) from None",
        "            downloaded = partial.stat().st_size if partial.exists() else 0",
        "            delay = min(20, backoff*2**(attempt-1))",
        "            aux_marker('HUMO17_AUX_DOWNLOAD_RETRY', a, attempt, existing, downloaded,",
        "                errorType=type(error).__name__, retryInSeconds=delay, retryAllowed=attempt < attempts)",
        "            if attempt == attempts:",
        "                raise RuntimeError('HUMO17_AUX_DOWNLOAD_EXHAUSTED:'+a['role']) from None",
        "            time.sleep(delay)"
    ].join("\n");
}
...
export function buildHuMo17RuntimeBootstrap(job) {
    ...
    const preparation = [
        buildHuMo17AuxiliaryDownloader(),
    ...
    return [
        ...
        "python -m pip install --disable-pip-version-check --force-reinstall --no-deps ninja==1.11.1.3 && python -m pip install --disable-pip-version-check -r ComfyUI/requirements.txt -r ComfyUI/custom_nodes/ComfyUI-WanVideoWrapper/requirements.txt soundfile transformers==4.51.3 tokenizers==0.21.4 peft==0.17.1 diffusers==0.33.1",
        "python -m pip check",
...
export async function runHuMo17PersistentCoreStagingCli({
    ...
    if (runtimeProbeAuthorized) {
        ...
        const audioInfo = JSON.parse(execFileSync(resolveLocalExecutable(env.JARVIS_FFPROBE_PATH || "ffprobe", env), ["-v", "error", "-show_streams", "-show_format", "-of", "json", audio.file], {encoding: "utf8", timeout: 20000, windowsHide: true}));
    ...
    const result = {
        ...
        persistentRuntimeReady: runtimePhysical?.runtimeReady === true,
...
export function resolveHuMo17QualityControlPlane({ root = DEFAULT_ROOT, env = process.env } = {}) {
    ...
    const durableCoreEvidencePath = path.resolve(root, ".sia7", "humo17-quality-201f-final-preflight.json");
    if (!fs.existsSync(durableCoreEvidencePath) || !fs.statSync(durableCoreEvidencePath).isFile() || fs.lstatSync(durableCoreEvidencePath).isSymbolicLink()) throw new Error("HUMO17_QUALITY_DURABLE_CORE_EVIDENCE_MISSING");
    const durableCoreEvidence = JSON.parse(fs.readFileSync(durableCoreEvidencePath, "utf8"));
    const durableReceiptSha256 = String(durableCoreEvidence?.coreStageReceiptSha256 || "").trim().toLowerCase();
    if (durableCoreEvidence?.ok !== true || durableCoreEvidence?.status !== "HUMO17_RUNTIME_ZERO_COST_PREFLIGHT_READY" || durableCoreEvidence?.cheapCpuRecertificationRequired !== false || durableCoreEvidence?.resourceCreated !== false || durableCoreEvidence?.inferenceStarted !== false || Number(durableCoreEvidence?.activePods) !== 0 || Number(durableCoreEvidence?.paidStaleReceipts) !== 0 || durableCoreEvidence?.terminationVerified !== true || durableCoreEvidence?.networkVolumeRetained !== true || durableCoreEvidence?.networkVolumeId !== "1qm5wczocl" || durableCoreEvidence?.volumeId !== "1qm5wczocl" || durableCoreEvidence?.dataCenterId !== "EU-NL-1" || durableCoreEvidence?.coreManifestVerified !== true || durableCoreEvidence?.coreStagePodId !== stageReceipt?.podId || !/^[a-f0-9]{64}$/.test(durableReceiptSha256)) throw new Error("HUMO17_QUALITY_DURABLE_CORE_EVIDENCE_INVALID");
    if (stageReceipt?.ok !== true || stageReceipt?.status !== "HUMO17_PERSISTENT_CORE_STAGED_AND_RELEASED" || stageReceipt?.physicalStageCertified !== true || stageReceipt?.coreManifestVerified !== true || stageReceipt?.terminationVerified !== true || stageReceipt?.networkVolumeRetained !== true || stageReceipt?.networkVolumeId !== "1qm5wczocl" || stageReceipt?.networkVolumeDataCenterId !== "EU-NL-1") throw new Error("HUMO17_QUALITY_HISTORICAL_CORE_ANCHOR_INVALID");
    // Durable core evidence and the historical physical anchor are validated above; the mounted L40S re-verifies the current core manifest before inference.
...
export async function runHuMoIdentityProbeCli({
    ...
    const requestedHardBudgetUsd = Number(String(env.JARVIS_HUMO_IDENTITY_PROBE_HARD_BUDGET_USD || "3").trim());
    if (!Number.isFinite(requestedHardBudgetUsd) || requestedHardBudgetUsd <= 0 || requestedHardBudgetUsd > 3) {
    ...
    const job = {
        ...
        sourceReferenceFiles: [reference.file, ...(secondReference ? [secondReference.file] : [])],
    ...
    return {
        ...
        networkVolumeId: persistentVolume.id,
        networkVolumeDataCenterId: persistentVolume.dataCenterId,
        networkVolumeSizeGb: Number(persistentVolume.sizeGb || 0),
        networkVolumeRetained: releaseReceipt?.networkVolumeRetained === true,
        estimatedMonthlyStorageUsd: 3.5,
