...
export function buildPageArtifactHtml(input = {}) {
    ...
    if (!whatsapp && !contactEmail.includes("@") && !whatsappRequested) throw new Error("PAGE_CONTACT_ROUTE_REQUIRED");
    ...
    const beforeAfterMarkup = beforeAfter.length ? `<section id="resultados"><div class="wrap"><p class="eyebrow">Antes y después</p><h2>Resultados visibles</h2><div class="before-after">${beforeAfter.map(item => `<figure><div><img loading="lazy" src="${escapeHtml(item.before)}" alt="Antes: ${escapeHtml(item.label)}"><span>Antes</span></div><div><img loading="lazy" src="${escapeHtml(item.after)}" alt="Después: ${escapeHtml(item.label)}"><span>Después</span></div><figcaption>${escapeHtml(item.label)}</figcaption></figure>`).join("")}</div></div></section>` : "";
    const contactAction = phoneHref || "#contacto";
    const structuredData = JSON.stringify({ "@context": "https://schema.org", "@type": "LocalBusiness", name: brandName, description, url: canonicalUrl || undefined, telephone: whatsapp || undefined }).replaceAll("<", "\\u003c");
    return `<!doctype html>
...
<body><a class="skip" href="#contenido">Saltar al contenido</a><header><nav class="nav wrap" aria-label="Principal"><a class="brand" href="#">${escapeHtml(brandName)}</a><button class="menu" aria-label="Abrir menú" aria-expanded="false">☰</button><div class="links"><a href="#servicios">Servicios</a>${gallery.length ? '<a href="#galeria">Galería</a>' : ""}${testimonials.length ? '<a href="#testimonios">Testimonios</a>' : ""}<a class="button" href="${escapeHtml(contactAction)}">Contactar</a></div></nav></header><main id="contenido"><section class="hero"><div class="hero-grid wrap"><div><p class="eyebrow">${escapeHtml(brandName)}</p><h1>${escapeHtml(title)}</h1><p>${escapeHtml(description)}</p><a class="button accent" href="${escapeHtml(contactAction)}">Solicitar información</a></div><div class="hero-media" role="img" aria-label="${escapeHtml(text(input.heroAlt, `Presentación de ${brandName}`))}"></div></div></section><section id="servicios"><div class="wrap"><p class="eyebrow">Servicios</p><h2>Soluciones diseñadas para resolver</h2><div class="cards">${serviceCards}</div></div></section>${galleryMarkup}${beforeAfterMarkup}${testimonialsMarkup}<section class="cta" id="contacto"><div class="wrap"><p class="eyebrow">Contacto</p><h2>Cuéntanos qué necesitas</h2><p>Responderemos con claridad y una propuesta adecuada a tu proyecto.</p><form class="contact-form"><label>Nombre<input name="name" autocomplete="name" required></label><label>Mensaje<textarea name="message" required></textarea></label><button class="button accent" type="submit">Enviar consulta</button></form>${mapUrl ? `<p><a href="${escapeHtml(mapUrl)}" target="_blank" rel="noopener">Ver ubicación en el mapa</a></p>` : ""}</div></section></main><footer class="site"><div class="wrap">© <span id="year"></span> ${escapeHtml(brandName)}. Todos los derechos reservados.</div></footer>
<script>const menu=document.querySelector('.menu'),links=document.querySelector('.links');menu.addEventListener('click',()=>{const open=links.classList.toggle('open');menu.setAttribute('aria-expanded',String(open))});document.querySelector('.contact-form').addEventListener('submit',event=>{event.preventDefault();const data=new FormData(event.currentTarget),message='Hola, soy '+data.get('name')+'. '+data.get('message');${phoneHref ? `window.open('${phoneHref}?text='+encodeURIComponent(message),'_blank','noopener')` : `window.location.href='mailto:${escapeHtml(contactEmail)}?subject='+encodeURIComponent('Consulta desde el sitio')+'&body='+encodeURIComponent(message)`}});document.getElementById('year').textContent=new Date().getFullYear();</script></body></html>`;
}
...
export function describePageArtifact(input = {}, html = "") {
    return {
        ...
        checks: {
            ...
            contact: html.includes('id="contacto"'),
            noTodoMarkers: !html.includes("TODO") && !html.includes("Lorem ipsum")
        },
