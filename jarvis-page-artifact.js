...
export function buildPageArtifactHtml(input = {}) {
    ...
    if (!whatsapp && !contactEmail.includes("@")) throw new Error("PAGE_CONTACT_ROUTE_REQUIRED");
...
<script>const menu=document.querySelector('.menu'),links=document.querySelector('.links');menu.addEventListener('click',()=>{const open=links.classList.toggle('open');menu.setAttribute('aria-expanded',String(open))});document.querySelector('.contact-form').addEventListener('submit',event=>{event.preventDefault();const data=new FormData(event.currentTarget),message='Hola, soy '+data.get('name')+'. '+data.get('message');${whatsapp ? `window.open('${phoneHref}?text='+encodeURIComponent(message),'_blank','noopener')` : `window.location.href='mailto:${escapeHtml(contactEmail)}?subject='+encodeURIComponent('Consulta desde el sitio')+'&body='+encodeURIComponent(message)`}});document.getElementById('year').textContent=new Date().getFullYear();</script></body></html>`;
}
