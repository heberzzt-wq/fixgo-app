...
if (
    !isStructured &&
    (
        cmd.startsWith("reparar dependencias ") ||
        cmd.startsWith("reparar dependencia ") ||
        cmd.startsWith("repair deps ") ||
        cmd.startsWith("repair dependencies ")
    )
) {

    const execute =
        /\b(ejecutar|execute|apply)\b/i.test(
            rawInput
        );

    const moduleName =
        rawInput
            .replace(/^reparar\s+dependencias?\s+/i, "")
            .replace(/^repair\s+(deps|dependencies)\s+/i, "")
            .replace(/\b(ejecutar|execute|apply)\b/ig, "")
            .trim();

    return await window.runDependencyRepairWorkflow?.({
        moduleName,
        preview:
            true,
        execute
...
if (
    ...
    return window.scanRepo?.({});
}

if (
    !isStructured &&
    (
        cmd.startsWith("impacto archivo ") ||
        cmd.startsWith("file impact ")
    )
) {

    const fileName =
        rawInput
            .replace(/^impacto\s+archivo\s+/i, "")
            .replace(/^file\s+impact\s+/i, "")
            .trim();

    return window.analyzeFileImpact?.(
        fileName
    );
}
...
if (
    !isStructured &&
    (
        cmd.startsWith("puede ejecutar ") ||
        cmd.startsWith("can execute ")
    )
) {

    const normalized =
        rawInput
            .replace(/^puede\s+ejecutar\s+/i, "")
            .replace(/^can\s+execute\s+/i, "")
            .trim();
    ...
    return window.canExecuteModuleOperation?.({
        moduleName,
        operation
    });
}
...
if (
    !isStructured &&
    (
        cmd.startsWith("cargar modulo ") ||
        cmd.startsWith("cargar módulo ") ||
        cmd.startsWith("load module ")
    )
) {

    const moduleName =
        rawInput
            .replace(/^cargar\s+m[oó]dulo\s+/i, "")
            .replace(/^load\s+module\s+/i, "")
            .trim();

    return await window.loadFirestoreModule?.(
        moduleName
    );
}
...
if (
    !isStructured &&
    (
        cmd === "restaurar runtime" ||
        cmd === "restore runtime" ||
        cmd === "restore cognition"
    )
) {

    return await window.restoreRuntimeCognition?.();
}

if (
    !isStructured &&
    (
        cmd.startsWith("auditar patch ") ||
        cmd.startsWith("patch audit ")
    )
) {

    const file =
        rawInput
            .replace(/^auditar\s+patch\s+/i, "")
            .replace(/^patch\s+audit\s+/i, "")
            .trim();

    return await window.runPatchWorkflow?.({
        file,
        strategy:
            "ANALYZE_ONLY",
        analysisResult: {
