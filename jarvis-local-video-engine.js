...
export function createRunpodRemoteVideoAdapter({
    ...
    function writeGpuRuntimeBootstrapFile(bootstrapFile) {
        ...
        const bootstrap = [
            ...
            `EPHEMERAL_MODEL_STAGING=${networkVolumeId ? "0" : "1"}`,
            ...
            "MODEL_CACHE_READY=0",
            `python3 "$MODEL_PREFLIGHT" ${shellSingleQuote(modelAuthorityJson)} "$MODEL_DIR" "$WAN_REPO" "$MODEL_MANIFEST" "$JARVIS_OPERATION_ID" && MODEL_CACHE_READY=1 || true`,
            "if test \"$MODEL_CACHE_READY\" != 1; then",
            "  if test \"$EPHEMERAL_MODEL_STAGING\" != 1; then printf 'RUNPOD_WAN22_PERSISTENT_CACHE_STAGING_REQUIRED\\n' >&2; exit 43; fi",
            "  progress MODEL_DOWNLOAD RUNNING CACHE_POPULATING",
            "  test -x \"$VENV/bin/hf\"",
