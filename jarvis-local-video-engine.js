...
export const NEXT_IDENTITY_RUNTIME_CANDIDATES = Object.freeze({
    "humo-17b-identity": Object.freeze({
        ...
        singleGpuStrategy: Object.freeze({
            ...
            quantizedModel: Object.freeze({
                repository: "Kijai/WanVideo_comfy_fp8_scaled",
                revision: "0d987e66e7debb098970adb907d99c07c5aa2ac9",
                path: "HuMo/Wan2_1-HuMo-14B_fp8_e4m3fn_scaled_KJ.safetensors",
                bytes: 17892294098,
            ...
            persistentMinimumNetworkVolumeGb: 50,
            persistentVolumeBytes: 53687091200,
...
export const RUNPOD_HUMO17_CORE_CACHE_BASE = (() => {
    ...
    return Object.freeze({
        ...
        provisionImageTag: RUNPOD_WAN22_CACHE_BASE.provisionImageTag,
        expectedRegistryDigest: RUNPOD_WAN22_CACHE_BASE.expectedRegistryDigest,
        operatingSystem: RUNPOD_WAN22_CACHE_BASE.runtimeIdentity.operatingSystem,
        pythonVersionPrefix: RUNPOD_WAN22_CACHE_BASE.pythonVersionPrefix,
        torchVersionPrefix: RUNPOD_WAN22_CACHE_BASE.torchVersionPrefix,
        torchCudaVersionPrefix: RUNPOD_WAN22_CACHE_BASE.torchCudaVersionPrefix,
...
export function buildHuMo17PersistentCoreStagingBootstrap({
    ...
    const script = [
        ...
        "manifest={key:contract[key] for key in ['schemaVersion','profile','runtime','targetGpuTypeId','provisionImageTag','expectedRegistryDigest','operatingSystem','pythonVersionPrefix','torchVersionPrefix','torchCudaVersionPrefix','comfyUiRepository','comfyUiRevision','wrapperRepository','wrapperRevision','modelRepository','modelRevision','existingHuMoCacheProfile','existingHuMoCacheTotalBytes','minimumNetworkVolumeGb','networkVolumeType','totalBytes','combinedPersistentBytes','nominalVolumeBytes','headroomBytes','storagePlan','cacheMutationPolicy']}",
        ...
        "cp -L /usr/bin/ffmpeg /usr/bin/ffprobe /usr/bin/git \"$SYSTEM_ROOT/bin/\"",
        "cp -a /usr/lib/git-core/. \"$SYSTEM_ROOT/lib/git-core/\"",
        "ldd /usr/bin/ffmpeg /usr/bin/ffprobe /usr/bin/git | awk '{for(i=1;i<=NF;i++) if($i ~ /^\\//) print $i}' | sort -u | while read -r lib; do test -f \"$lib\" && cp -L \"$lib\" \"$SYSTEM_ROOT/lib/\"; done",
        "test -x \"$SYSTEM_ROOT/bin/ffmpeg\" && test -x \"$SYSTEM_ROOT/bin/ffprobe\" && test -x \"$SYSTEM_ROOT/bin/git\"",
        "\"$RUNTIME_ROOT/venv/bin/python\" - \"$CORE_ROOT\" \"$RUNTIME_ROOT\" \"$NETWORK_VOLUME_ID\" \"$DATA_CENTER_ID\" \"$CONTRACT_B64\" <<'PYRUNTIME'",
        "import base64,datetime,hashlib,json,os,pathlib,subprocess,sys,tempfile,torch",
        "core=pathlib.Path(sys.argv[1]); runtime=pathlib.Path(sys.argv[2]); volume_id=sys.argv[3]; dc=sys.argv[4]; contract=json.loads(base64.b64decode(sys.argv[5]).decode('utf-8')); comfy=runtime/'ComfyUI'",
        "def git_head(repo): return subprocess.check_output(['git','-C',str(repo),'rev-parse','HEAD'],text=True).strip()",
        "if git_head(comfy)!=contract['comfyUiRevision']: raise RuntimeError('HUMO17_PERSISTENT_COMFY_REVISION_MISMATCH')",
        "wrapper=comfy/'custom_nodes'/'ComfyUI-WanVideoWrapper'",
        "if git_head(wrapper)!=contract['wrapperRevision']: raise RuntimeError('HUMO17_PERSISTENT_WRAPPER_REVISION_MISMATCH')",
        ...
        "for item in contract['requiredFiles']:",
        "    source=core/item['path']; target=comfy/'models'/folders[item['role']]/pathlib.Path(item['sourcePath']).name; target.parent.mkdir(parents=True,exist_ok=True)",
        "    if target.exists() or target.is_symlink(): target.unlink()",
        "    target.symlink_to(source)",
        "    if not source.is_file() or target.resolve()!=source.resolve(): raise RuntimeError('HUMO17_PERSISTENT_MODEL_LINK_INVALID:'+item['role'])",
        "core_manifest=core/'model-manifest.json'; core_sha=hashlib.sha256(core_manifest.read_bytes()).hexdigest()",
        "manifest={'schema':'jarvis.v142.humo17-persistent-runtime.1','runtimeReady':True,'verifiedAt':datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z'),'networkVolumeId':volume_id,'dataCenterId':dc,'provisionImageTag':contract['provisionImageTag'],'expectedRegistryDigest':contract['expectedRegistryDigest'],'operatingSystem':contract['operatingSystem'],'pythonVersionPrefix':contract['pythonVersionPrefix'],'torchVersionPrefix':contract['torchVersionPrefix'],'torchCudaVersionPrefix':contract['torchCudaVersionPrefix'],'comfyUiRevision':contract['comfyUiRevision'],'wrapperRevision':contract['wrapperRevision'],'pythonVersion':sys.version.split()[0],'torchVersion':torch.__version__,'torchCudaVersion':str(torch.version.cuda or ''),'coreManifestSha256':core_sha,'requiredAssetCount':len(contract['requiredFiles']),'systemToolsReady':True,'ffmpegPath':str(runtime/'system'/'bin'/'ffmpeg'),'ffprobePath':str(runtime/'system'/'bin'/'ffprobe'),'offlinePaidBootstrapRequired':True}",
        "target=runtime/'runtime-manifest.json'; fd,tmp=tempfile.mkstemp(prefix='.runtime-manifest-',dir=runtime); os.close(fd)",
        "with open(tmp,'w',encoding='utf-8') as stream: json.dump(manifest,stream,sort_keys=True,separators=(',',':')); stream.write('\\n'); stream.flush(); os.fsync(stream.fileno())",
        "os.replace(tmp,target); print(json.dumps(manifest,separators=(',',':')))",
        "PYRUNTIME",
...
export function buildHuMoLocalEphemeralTransferPlan({
    ...
    return {
        ...
        sourceRepositoryPath: path.join(sourceRoot, "HuMo"),
        destinationSourceRepository: `${destinationCacheRoot}/HuMo`,
...
export function createRunpodRemoteVideoAdapter({
    ...
    stageHuMoLocalCacheToEphemeralImpl = null,
    now = () => new Date()
} = {}) {
    const resolvedRoot = path.resolve(root);
    ...
    function huMoIdentityProbeExecutionAuthorized(job = null) {
        ...
        return (
            ...
            durationSeconds > 0 &&
            durationSeconds <= RUNPOD_HUMO_IDENTITY_CANDIDATE.candidateProbeGeometry.durationSeconds + 0.001
        );
    }
    ...
    function writeHuMoRuntimeBootstrapFile(bootstrapFile) {
        ...
        const bootstrap = [
            ...
            "if test \"$POPULATE_PERSISTENT_HUMO_CACHE\" = 1; then",
    ...
    return {
        ...
        inspectPlacementInventory: queryPlacementInventory,
