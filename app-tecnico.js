...
window.aceptarServicio = async (id) => {
    ...
    try {
        ...
        console.error(e);
        alert("Error: No se pudo aceptar el servicio.");
    }
};
...
function escucharMiServicioActivo(uid) {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        if (!servicioActivo) {
            ...
            const serv = servicioActivo.data();
            const servId = servicioActivo.id;
    ...
    }, (error) => console.error("Error en Servicio Activo:", error));
}
