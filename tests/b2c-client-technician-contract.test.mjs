...
test("integración elimina overrides silenciosos, amplía mapa y delega aprobación", () => {
    ...
    assert.match(client, /legacyManualReviewMigration/);
    assert.match(client, /FINALIZAR VALIDACIÓN AUTOMÁTICA/);
    assert.match(client, /queueMicrotask\(\(\) => retryButton\.click\(\)\)/);
    assert.doesNotMatch(client, /Identidad en revisión administrativa/);
    assert.doesNotMatch(admin, /customerIdentityReviewQueue|approve_customer_identity|request_customer_identity_recapture/);
