...
test("V142 HuMo17 single-L40S candidate pins FP8 block-swap authority and remains fail-closed", () => {
    ...
    assert.equal(strategy.comfyUiRevision, "b30e980a206607d1a9d56b7a6f7df3999d68438a");
    ...
    assert.equal(strategy.baseAndDiffusionPersistentBytes, 39987403600);
    assert.equal(strategy.distillationLoraBytes, 738005744);
    assert.equal(strategy.persistentCoreBytes, 40725409344);
    assert.equal(strategy.wrapperAuxiliaryBytes, 13309522014);
    assert.equal(strategy.persistentCoreHeadroomBytes, 12961681856);
    assert.equal(strategy.ephemeralWorkspaceReserveBytes, 8589934592);
    assert.equal(strategy.minimumEphemeralBytes, 21899456606);
...
test("V142 HuMo Whisper dtype repair is transient and follows the physical encoder dtype", () => {
    ...
    assert.equal(runner.includes('\"frame_count\": 97'), true);
    assert.equal(runner.includes('\"probe_duration_seconds\": 3.88'), true);
    ...
    const fetchImpl = async (url, options = {}) => {
        ...
        if (String(url).startsWith("https://registry-1.docker.io/v2/")) {
            ...
            const expectedDigest = String(url).includes("/library/ubuntu/")
                ...
                : String(url).includes("2.5.1-cuda12.4-cudnn9-devel")
...
test("V142 HuMo is explicit-only and fail-closed until paid identity execution authorization", () => {
    ...
    assert.equal(candidate.includes("pytorch/pytorch:2.5.1-cuda12.4-cudnn9-devel"), true);
    assert.equal(candidate.includes("14611869895df612b7b07227d5925f30ec3cd6673bad58ce3d84ed107950e014"), true);
    ...
    assert.equal(candidate.includes("physicalRuntimeCertification: Object.freeze("), true);
    ...
    try {
        ...
        const report = adapter.inspectHuMoZeroCostPrecheck({
            registryVerification: {
                ...
                expectedDigest: "sha256:14611869895df612b7b07227d5925f30ec3cd6673bad58ce3d84ed107950e014",
                observedDigest: "sha256:14611869895df612b7b07227d5925f30ec3cd6673bad58ce3d84ed107950e014",
...
for (const fault of [null, ...["base", "full"].flatMap(stage => [
    ["pythonVersion", "3.10.12"], ["torchVersion", "2.4.0"],
    ["torchCudaVersion", "12.1"],
    ["cuda", false], ["gpuName", "NVIDIA L40"], ["vramBytes", 40 * 1024 ** 3]
].map(([field, value]) => ({ stage, field, value }))),
...["flashAttentionVersion", "sourceRevision", "pipCheck", "dependencyContract"].map(field => ({ stage: "full", field, value: false }))]) {
test("V142 HuMo strict base and bootstrap 3.11 cu124: " + (fault ? fault.stage + " rejects " + fault.field : "certifies without inference"), async () => {
    ...
    const harness = runpodPhysicalHarness({
        ...
        baseHealthOverrides: {
            ...
            ffmpeg: true,
            ffprobe: true,
            ...(fault?.stage === "base" ? { [fault.field]: fault.value } : {})
        },
        runtimeHealthOverrides: {
            operatingSystem: "ubuntu-22.04",
            pythonVersion: "3.11.9",
            ...
            ...(fault?.stage === "full" ? { [fault.field]: fault.value } : {})
        }
    });
    const job = {
        ...harness.dryRunJob,
    ...
    assert.equal(harness.createdBody.imageName, "pytorch/pytorch:2.5.1-cuda12.4-cudnn9-devel");
    assert.equal(harness.createdBody.cloudType, "SECURE");
    assert.equal(harness.createdGraphQlInput.imageName, harness.createdBody.imageName);
    assert.ok(harness.createdGraphQlInput.dockerArgs.includes("sshd -D -e"));
    ...
    if (fault) {
        assert.equal(certified?.ok, false, JSON.stringify(certified));
        assert.equal(harness.inferenceStarts, 0);
        const state = JSON.parse(fs.readFileSync(path.join(harness.root, ".jarvis-artifacts", ".video-worker", "runpod", job.operationId + ".json"), "utf8"));
        assert.ok(state.runtimePredicateFailures.includes(fault.field === "cuda" ? "cudaAvailable" : fault.field === "vramBytes" ? "vramObserved" : fault.field));
        const released = await harness.adapter.release({ ...job, remoteWorker: launched.remoteWorker, reason: "failed_runtime_certification" });
        assert.equal(released.terminationVerified, true);
        assert.equal(harness.deleted, true);
        return;
    }
    assert.equal(certified?.ok, true, JSON.stringify(certified));
    ...
    assert.ok(harness.calls.some(call => call.kind === "ssh" && call.command.startsWith("'/opt/conda/bin/python' -c")));
...
test("V142 HuMo preinstalled base runtime requires fresh physical certification", () => {
    ...
    assert.equal(engineSource.includes('baseCuda: "12.4.1"'), true);
    assert.equal(engineSource.includes('physicalRuntimeCertified: true'), true);
    assert.equal(engineSource.includes('physicalRuntimeCertification: Object.freeze('), true);
});
...
test("V142 HuMo physical runtime receipt is current and preserves unpaid identity and portrait gates", () => {
    const source = fs.readFileSync(new URL("../jarvis-local-video-engine.js", import.meta.url), "utf8");
    const start = source.indexOf("const RUNPOD_HUMO_IDENTITY_CANDIDATE = Object.freeze({");
    ...
    assert.match(candidate, /runtimeCertificationOnly["\s]*: true/);
    assert.match(candidate, /inferenceStarted["\s]*: false/);
    assert.match(candidate, /terminationVerified["\s]*: true/);
    assert.match(candidate, /physicalRuntimeCertification: Object\.freeze\(/);
    assert.match(candidate, /paidExecutionAuthorized: false/);
    assert.match(candidate, /physicalPortraitCertified: false/);
});
...
test("V142 SIA7 HuMo executor is typed, budget-capped, source-pinned and control-plane-only after certification", () => {
    ...
    assert.equal(workerSource.includes("Downloads\", \"humo-heberto-minidrama-good-voice-8s.wav"), true);
    assert.equal(workerSource.includes("SIA7_HUMO_GOOD_VOICE_DOWNLOAD_BYTES_INVALID"), true);
    assert.equal(workerSource.includes("SIA7_HUMO_GOOD_VOICE_DOWNLOAD_SHA_MISMATCH"), true);
    assert.equal(workerSource.includes("materializationSource: \"verified_user_download\""), true);
...
test("V142 HuMo runtime certification stays ephemeral while identity CLI uses a retained network volume", () => {
    ...
    assert.doesNotMatch(identityCli, /resolveHuMoLanCacheAuthority/);
    assert.match(identityCli, /ensureHuMoPersistentNetworkVolume/);
    assert.equal(bridgeSource.includes("JARVIS_RUNPOD_NETWORK_VOLUME_API_BASE || \"https://rest.runpod.io/v1\""), true);
...
test("V142 HuMo paid bootstrap explicitly reuses the official Conda Python Torch runtime and invalidates stale physical certification", () => {
    const engine = fs.readFileSync(new URL("../jarvis-local-video-engine.js", import.meta.url), "utf8");
    assert.equal(engine.includes("pytorch/pytorch:2.5.1-cuda12.4-cudnn9-devel"), true);
    assert.equal(engine.includes("sha256:14611869895df612b7b07227d5925f30ec3cd6673bad58ce3d84ed107950e014"), true);
    ...
    assert.equal(humoPreFlash.includes("python3 -m venv --system-site-packages"), false);
    assert.match(humoPreFlash, /basePythonExecutable/);
    assert.match(humoPreFlash, /--system-site-packages/);
    assert.equal(humoPreFlash.includes("importlib.metadata.version('torch').startswith('2.5.1')"), true);
    assert.equal(engine.includes("runtimePreflightCertified: true"), true);
    assert.equal(engine.includes("physicalRuntimeCertified: true"), true);
    assert.equal(engine.includes("physicalRuntimeCertification: Object.freeze("), true);
...
test("V142 GraphQL provision connect timeout is never retried automatically", async () => {
 ...
 test("V142 HuMo runtime certification receipt binds the physical image and official runtime", () => {
 const a=buildHuMoIdentityRuntimeAuthority();const r=a.physicalRuntimeCertification;
 assert.equal(r.canonicalSha,"fb973d73fc66ae60664302f21edc5e84e31f9885");
 assert.equal(r.operationName,"local-video/72e89488-b552-4d3f-b712-82db76d5eae1");
 assert.equal(r.provisionImageTag,a.remoteRuntimeBase.provisionImageTag);
 assert.equal(r.expectedRegistryDigest,a.remoteRuntimeBase.expectedRegistryDigest);
 assert.equal(r.sourceRevision,a.sourceRevision);assert.equal(r.terminationVerified,true);
 assert.equal(r.inferenceStarted,false);assert.equal(r.runtimeCertificationOnly,true);
