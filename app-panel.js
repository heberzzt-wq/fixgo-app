 ...
 * Versión: 3.5 (Stable: Admin Fix + Tech Button Simplification)
 * ======================================================
 */
...
console.log("🧩 app-panel.js V3.5: Sistema Estable y Entrelazado.");
...
export async function iniciarPanelAdmin(user) {
    ...
    // Buscamos los contadores del dashboard
    ...
    // 1.A. LISTA DE APROBACIÓN DE TÉCNICOS
    if (contenedorTecnicos) {
        ...
        onSnapshot(qTecnicos, (snapshot) => {
            ...
            if (snapshot.empty) {
                contenedorTecnicos.innerHTML = '<p class="text-gray-500 italic p-4">No hay técnicos.</p>';
                return;
            }
    ...
    // 1.C. SERVICIOS ACTIVOS Y ACTIVIDAD RECIENTE (¡Aquí estaba lo que faltaba!)
    ...
    onSnapshot(qServicios, (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
            ...
            if (data.precio_final) ingresos += (data.precio_final * 0.32);

            // Renderizar lista (limitada a 10 visualmente)
        ...
        if(contadorServicios) {
            ...
            contadorServicios.style.color = activos > 0 ? "#34d399" : "white";
        }
...
export async function iniciarPanelTecnico(user) {
    ...
    function escucharBolsaDeTrabajo(tecnico) {
        ...
        onSnapshot(qBolsa, (snap) => {
            ...
            if(snap.empty) {
                listaBolsa.innerHTML = `<p class="text-gray-600 text-[10px] text-center italic">Escaneando zona...</p>`;
                return;
            }
    ...
    // 2.C. MISIONES ACTIVAS (CORRECCIÓN CRÍTICA DE BOTONES)
    ...
    onSnapshot(qMisiones, (snap) => {
        ...
        snap.forEach((docSnap) => {
            ...
            // GESTIÓN DE BOTONES SIMPLIFICADA (SIN CLONE NODE)
            // Primero ocultamos ambos
            if(btnEnCamino) btnEnCamino.classList.add("hidden");
            if(btnLlegue) btnLlegue.classList.add("hidden");

            // Activamos según estado
            if (servicio.estado === "asignado") {
                if(btnEnCamino) {
                    btnEnCamino.classList.remove("hidden");
                    // ASIGNACIÓN DIRECTA DEL CLICK (Sobrescribe la anterior, no acumula)
                    btnEnCamino.onclick = function() { actualizarEstadoServicio(servicioId, "en_camino"); };
                }
            } 
                ...
                if(btnLlegue) {
                    ...
                    btnLlegue.className = "w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-xl text-lg flex items-center justify-center gap-3";
                    btnLlegue.onclick = function() { actualizarEstadoServicio(servicioId, "en_sitio"); };
                }
            }
                ...
                if(btnLlegue) {
                    ...
                    btnLlegue.className = "w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-xl text-lg flex items-center justify-center gap-3";
                    btnLlegue.onclick = function() { alert("Fase de cotización: Próximamente"); };
                }
            }
        });
    });
