...
</style></head><body><main class="studio"><section class="panel"><p class="eyebrow">JARVIS · REEL STUDIO V9</p><h1>${escapeHtml(title)}</h1><p>${escapeHtml(brandName)} · ${durationSeconds} segundos · 1080 × 1920 · 9:16</p><ul class="facts"><li>Timeline editable en el objeto de campaña</li><li>Preview real en canvas</li><li>Medios fuente precargados, transiciones efectivas, subtítulos, logotipo y CTA</li><li>Audio adjunto prioritario o audio de video fuente cuando exista</li><li>Exportación WebM local con control de carga y SHA-256</li></ul><div class="actions"><button id="play">Previsualizar</button><button class="primary" id="export">Exportar WebM</button><a class="download" id="videoDownload" hidden>Descargar video</a></div><p class="status" id="status" aria-live="polite"></p><div class="timeline" id="timeline"></div></section><div class="phone"><canvas id="reel" width="1080" height="1920" aria-label="Vista previa vertical del reel"></canvas></div></main>
...
document.querySelector('#export').onclick=async event=>{if(exporting)return;if(!window.MediaRecorder||!canvas.captureStream){statusEl.textContent='Este navegador no permite exportar WebM.';return}exporting=true;event.currentTarget.disabled=true;const readiness=await waitForMediaReady();const qualityGatePassed=readiness.mediaFailedCount===0&&(!readiness.audioExpected||readiness.audioReady===true);if(!qualityGatePassed){window.__JARVIS_REEL_EXPORT_ERROR__={status:'REEL_SOURCE_MEDIA_NOT_READY',...readiness};statusEl.textContent='Exportación bloqueada: los medios requeridos no están listos.';event.currentTarget.disabled=false;exporting=false;return}const stream=canvas.captureStream(30),audioRouting=await attachExportAudioTracks(stream);const mime=['video/webm;codecs=vp9','video/webm;codecs=vp8','video/webm'].find(type=>MediaRecorder.isTypeSupported(type))||'',recorder=new MediaRecorder(stream,mime?{mimeType:mime}:undefined),chunks=[];recorder.ondataavailable=chunk=>chunk.data.size&&chunks.push(chunk.data);recorder.start(250);const started=await play();if(!started){recorder.stop();event.currentTarget.disabled=false;exporting=false;return}const ticker=setInterval(()=>{statusEl.textContent='Exportando '+Math.min(spec.durationSeconds,Math.floor((performance.now()-startedAt)/1000))+' de '+spec.durationSeconds+' segundos…'},500);await new Promise(resolve=>setTimeout(resolve,spec.durationSeconds*1000+300));recorder.stop();await new Promise(resolve=>recorder.onstop=resolve);clearInterval(ticker);const blob=new Blob(chunks,{type:mime||'video/webm'}),hash=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',await blob.arrayBuffer()))).map(byte=>byte.toString(16).padStart(2,'0')).join(''),url=URL.createObjectURL(blob),link=document.querySelector('#videoDownload'),detail={durationSeconds:spec.durationSeconds,width:1080,height:1920,mimeType:blob.type,bytes:blob.size,sha256:hash,fileName:'jarvis-reel-'+spec.durationSeconds+'s.webm',qualityGatePassed,...readiness,...audioRouting};window.__JARVIS_LAST_REEL_BLOB__=blob;window.__JARVIS_LAST_REEL_DETAIL__=detail;link.href=url;link.download=detail.fileName;link.hidden=false;window.dispatchEvent(new CustomEvent('jarvis:reel-exported',{detail}));statusEl.textContent='WebM verificado: '+blob.size+' bytes · SHA-256 '+hash.slice(0,12)+'…';event.currentTarget.disabled=false;exporting=false;if(!window.__JARVIS_HEADLESS_EXPORT__)setTimeout(()=>link.click(),0)};draw(0);
