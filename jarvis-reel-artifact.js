...
// V142 deterministic baseline compatibility marker (inactive): exporting?setTimeout(()=>frame(performance.now()),1000/30):requestAnimationFrame(frame)
export function buildReelStudioHtml(input = {}) {
    ...
    return `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)} Â· Reel Studio Jarvis</title><style>
...
</style></head><body><main class="studio"><section class="panel"><p class="eyebrow">JARVIS Â· REEL STUDIO V9</p><h1>${escapeHtml(title)}</h1><p>${escapeHtml(brandName)} Â· ${durationSeconds} segundos Â· 1080 Ã— 1920 Â· 9:16</p><ul class="facts"><li>Timeline editable en el objeto de campaÃ±a</li><li>Preview real en canvas</li><li>Medios fuente precargados, transiciones efectivas, subtÃ­tulos, logotipo y CTA</li><li>Audio adjunto prioritario o audio de video fuente cuando exista</li><li>ExportaciÃ³n MP4 H.264/AAC preferida, fallback WebM y SHA-256</li></ul><div class="actions"><button id="play">Previsualizar</button><button class="primary" id="export">Exportar video</button><a class="download" id="videoDownload" hidden>Descargar video</a></div><p class="status" id="status" aria-live="polite"></p><div class="timeline" id="timeline"></div></section><div class="phone"><canvas id="reel" width="1080" height="1920" aria-label="Vista previa vertical del reel"></canvas></div></main>
<script type="application/json" id="reelData">${payload}</script><script>
const spec=JSON.parse(document.querySelector('#reelData').textContent),canvas=document.querySelector('#reel'),ctx=canvas.getContext('2d',{alpha:false,desynchronized:true}),statusEl=document.querySelector('#status'),timeline=document.querySelector('#timeline');let animation=0,startedAt=0,exporting=false,renderedFrameCount=0,mediaReadinessPromise=null,audioGraphPromise=null,activeSceneId=0,brandWidthCache=0,ctaWidthCache=0;const headlineCache=new Map(),subtitleCache=new Map();const media=spec.scenes.map(scene=>{if(!scene.mediaUrl)return null;if(scene.mediaType==='video'||scene.mediaUrl.startsWith('data:video/')){const item=document.createElement('video');item.src=scene.mediaUrl;item.muted=true;item.playsInline=true;item.preload='auto';item.loop=true;if(scene.mediaUrl.startsWith('https:'))item.crossOrigin='anonymous';return item}const item=new Image();item.crossOrigin='anonymous';item.src=scene.mediaUrl;return item});const logo=spec.logoUrl?Object.assign(new Image(),{src:spec.logoUrl}):null,audio=spec.audioUrl?Object.assign(new Audio(),{src:spec.audioUrl,preload:'auto'}):null;const topShade=ctx.createLinearGradient(0,0,0,620);topShade.addColorStop(0,'#000c');topShade.addColorStop(1,'#0000');const lowerShade=ctx.createLinearGradient(0,760,0,1920);lowerShade.addColorStop(0,'#0000');lowerShade.addColorStop(.34,'#0006');lowerShade.addColorStop(1,'#000f');
spec.scenes.forEach(scene=>{const item=document.createElement('div');item.className='scene';item.dataset.scene=scene.id;item.innerHTML='<strong>'+String(scene.id).padStart(2,'0')+'</strong><span>'+escapeText(scene.overlay)+'</span><small>'+scene.durationSeconds+'s</small>';timeline.appendChild(item)});const timelineNodes=Array.from(timeline.querySelectorAll('.scene'));function escapeText(value){const element=document.createElement('span');element.textContent=value;return element.innerHTML}function sceneAt(seconds){let cursor=0;for(const scene of spec.scenes){if(seconds<cursor+scene.durationSeconds)return{scene,start:cursor};cursor+=scene.durationSeconds}return{scene:spec.scenes.at(-1),start:spec.durationSeconds-spec.scenes.at(-1).durationSeconds}}
...
async function probeMediaReadiness(){const visualItems=media.filter(Boolean);if(logo)visualItems.push(logo);const visualResults=await Promise.all(visualItems.map((item,index)=>waitForVisualItem(item,index)));const sourceMediaCount=visualItems.length,mediaReadyCount=visualResults.filter(item=>item.ready).length,mediaFailedCount=sourceMediaCount-mediaReadyCount,audioExpected=Boolean(audio),audioReady=await waitForAudio();return{sourceMediaCount,mediaReadyCount,mediaFailedCount,audioExpected,audioReady,failedMediaIndexes:visualResults.filter(item=>!item.ready).map(item=>item.index)}}
async function waitForMediaReady(){if(mediaReadinessPromise)return mediaReadinessPromise;mediaReadinessPromise=(async()=>{let detail=await probeMediaReadiness();if(detail.mediaFailedCount>0||(detail.audioExpected&&!detail.audioReady)){await new Promise(resolve=>setTimeout(resolve,1200));detail=await probeMediaReadiness()}window.__JARVIS_REEL_MEDIA_READINESS__=detail;return detail})();const detail=await mediaReadinessPromise;if(detail.mediaFailedCount>0||(detail.audioExpected&&!detail.audioReady))mediaReadinessPromise=null;return detail}
...
function fitHeadline(value,maxWidth=930,maxLines=5){const key=String(value)+'|'+maxWidth+'|'+maxLines;if(headlineCache.has(key))return headlineCache.get(key);for(let size=84;size>=54;size-=4){ctx.font='900 '+size+'px system-ui';const lines=wrapText(value,maxWidth);if(lines.length<=maxLines){const result={size,lines};headlineCache.set(key,result);return result}}ctx.font='900 54px system-ui';const result={size:54,lines:wrapText(value,maxWidth).slice(0,maxLines)};headlineCache.set(key,result);return result}function subtitleLines(value,maxWidth=858){const key=String(value)+'|'+maxWidth;if(subtitleCache.has(key))return subtitleCache.get(key);ctx.font='650 39px system-ui';const lines=wrapText(value,maxWidth).slice(0,3);subtitleCache.set(key,lines);return lines}function applySceneTransition(scene,local,edge){const entry=easeOut(Math.min(1,local/.55));if(scene.transition==='cut')return 1;if(scene.transition==='slide'){ctx.translate((1-entry)*(scene.id%2===0?-85:85),0);return Math.max(.2,edge)}if(scene.transition==='zoom'){const scale=.965+.035*entry;ctx.translate(540,960);ctx.scale(scale,scale);ctx.translate(-540,-960);return Math.max(.25,edge)}if(scene.transition==='dissolve')return Math.max(.08,edge*edge);return Math.max(.12,edge)}function syncVideoPlayback(activeIndex){media.forEach((item,index)=>{if(!item||item.tagName!=='VIDEO')return;if(index!==activeIndex&&!item.paused)item.pause()})}
function draw(seconds){const state=sceneAt(Math.min(seconds,spec.durationSeconds-.001)),scene=state.scene,index=scene.id-1,item=media[index],local=Math.max(0,seconds-state.start),progress=Math.max(0,Math.min(1,local/scene.durationSeconds)),entry=easeOut(Math.min(1,local/.65)),edge=Math.min(1,local/.45,(scene.durationSeconds-local)/.45);syncVideoPlayback(index);ctx.globalAlpha=1;ctx.fillStyle=scene.backgroundColor;ctx.fillRect(0,0,1080,1920);if(item){ctx.save();ctx.globalAlpha=applySceneTransition(scene,local,edge);if(item.tagName==='VIDEO'){const sourceDuration=Number.isFinite(item.duration)&&item.duration>0?item.duration:null,targetTime=sourceDuration?local%sourceDuration:local;if(Math.abs(item.currentTime-targetTime)>.35)item.currentTime=targetTime;if(item.readyState>=2){item.play().catch(()=>{});cover(item,1.035+progress*.025,(progress-.5)*-18,(progress-.5)*-12)}}else if(item.complete&&item.naturalWidth){cover(item,1.04+progress*.09,(progress-.5)*-34,(progress-.5)*-24)}ctx.restore()}ctx.fillStyle=topShade;ctx.fillRect(0,0,1080,700);ctx.fillStyle=lowerShade;ctx.fillRect(0,680,1080,1240);ctx.save();ctx.translate(0,(1-entry)*42);ctx.globalAlpha=entry;ctx.fillStyle='#07111fcc';roundedRect(55,54,Math.min(780,Math.max(330,brandMeasure()+88)),92,46);ctx.fill();ctx.fillStyle=spec.accentColor;ctx.font='900 37px system-ui';ctx.fillText(spec.brandName.toUpperCase(),92,113);ctx.restore();if(logo&&logo.complete){ctx.save();ctx.globalAlpha=.96;ctx.fillStyle='#ffffffee';roundedRect(850,44,165,165,32);ctx.fill();ctx.drawImage(logo,872,66,121,121);ctx.restore()}ctx.save();ctx.translate((1-entry)*52,0);ctx.globalAlpha=entry;ctx.shadowColor='#000';ctx.shadowBlur=exporting?8:30;ctx.fillStyle='#fff';const headline=fitHeadline(scene.overlay,930,5);ctx.font='900 '+headline.size+'px system-ui';const lineHeight=headline.size*1.16;headline.lines.forEach((line,lineIndex)=>ctx.fillText(line,70,1160+lineIndex*lineHeight));ctx.restore();const subtitle=scene.subtitle;if(subtitle){ctx.font='650 39px system-ui';const subtitles=subtitleLines(subtitle,858),boxHeight=56+subtitles.length*52;ctx.fillStyle='#050914d9';roundedRect(58,1590-boxHeight+50,930,boxHeight,28);ctx.fill();ctx.fillStyle='#eef6ff';subtitles.forEach((line,lineIndex)=>ctx.fillText(line,91,1590-boxHeight+105+lineIndex*52))}ctx.font='850 34px system-ui';const ctaWidth=measureCta();ctx.fillStyle=spec.accentColor;roundedRect(70,1745,ctaWidth,90,45);ctx.fill();ctx.fillStyle='#06120f';ctx.fillText(spec.cta,116,1802);ctx.fillStyle='#ffffff3d';ctx.fillRect(70,1870,940,8);ctx.fillStyle=spec.accentColor;ctx.fillRect(70,1870,940*Math.max(0,Math.min(1,seconds/spec.durationSeconds)),8);if(activeSceneId!==scene.id){activeSceneId=scene.id;timelineNodes.forEach(node=>node.classList.toggle('active',Number(node.dataset.scene)===scene.id))}}function brandMeasure(){if(brandWidthCache>0)return brandWidthCache;ctx.font='900 37px system-ui';brandWidthCache=ctx.measureText(spec.brandName.toUpperCase()).width;return brandWidthCache}function measureCta(){if(ctaWidthCache>0)return ctaWidthCache;ctx.font='850 34px system-ui';ctaWidthCache=Math.min(900,Math.max(330,ctx.measureText(spec.cta).width+92));return ctaWidthCache}
function scheduleFrame(frameStartedAt){if(!exporting){animation=requestAnimationFrame(frame);return}const targetDelay=1000/30,elapsed=Math.max(0,performance.now()-frameStartedAt);animation=setTimeout(()=>frame(performance.now()),Math.max(0,targetDelay-elapsed))}function frame(now){const frameStartedAt=performance.now(),seconds=(now-startedAt)/1000;draw(seconds);renderedFrameCount+=1;if(seconds<spec.durationSeconds){scheduleFrame(frameStartedAt)}else{draw(spec.durationSeconds-.001);renderedFrameCount+=1;media.forEach(item=>item?.tagName==='VIDEO'&&!item.paused&&item.pause());if(audio&&!audio.paused)audio.pause();statusEl.textContent='Vista previa finalizada.'}}async function play(){const readiness=await waitForMediaReady();if(readiness.mediaFailedCount>0){statusEl.textContent='No se puede previsualizar: faltan medios visuales por cargar.';return false}const audioGraph=await ensureAudioGraph();if(audioGraph.context?.state==='suspended')await audioGraph.context.resume().catch(()=>{});cancelAnimationFrame(animation);clearTimeout(animation);renderedFrameCount=0;activeSceneId=0;startedAt=performance.now();if(audio&&readiness.audioReady){audio.currentTime=0;audio.play().catch(()=>{})}animation=exporting?setTimeout(()=>frame(performance.now()),0):requestAnimationFrame(frame);return true}document.querySelector('#play').onclick=()=>{play()};
document.querySelector('#export').onclick=async event=>{if(exporting)return;if(!window.MediaRecorder||!canvas.captureStream){statusEl.textContent='Este navegador no permite exportar video.';return}exporting=true;window.__JARVIS_REEL_EXPORT_ERROR__=null;event.currentTarget.disabled=true;const readiness=await waitForMediaReady();const qualityGatePassed=readiness.mediaFailedCount===0&&(!readiness.audioExpected||readiness.audioReady===true);if(!qualityGatePassed){window.__JARVIS_REEL_EXPORT_ERROR__={status:'REEL_SOURCE_MEDIA_NOT_READY',...readiness};statusEl.textContent='ExportaciÃ³n bloqueada: los medios requeridos no estÃ¡n listos.';event.currentTarget.disabled=false;exporting=false;return}const stream=canvas.captureStream(30),audioRouting=await attachExportAudioTracks(stream),mp4Types=audioRouting.audioTracksAdded>0?['video/mp4;codecs=avc1.42E01E,mp4a.40.2','video/mp4;codecs=avc1.42E01E','video/mp4']:['video/mp4;codecs=avc1.42E01E','video/mp4'],fallbackTypes=['video/webm;codecs=vp9','video/webm;codecs=vp8','video/webm'],mime=[...mp4Types,...fallbackTypes].find(type=>MediaRecorder.isTypeSupported(type))||'',recorder=new MediaRecorder(stream,mime?{mimeType:mime}:undefined),chunks=[];recorder.ondataavailable=chunk=>chunk.data.size&&chunks.push(chunk.data);recorder.start(250);const started=await play();if(!started){recorder.stop();event.currentTarget.disabled=false;exporting=false;return}const ticker=setInterval(()=>{statusEl.textContent='Exportando '+Math.min(spec.durationSeconds,Math.floor((performance.now()-startedAt)/1000))+' de '+spec.durationSeconds+' segundosâ€¦'},500);await new Promise(resolve=>setTimeout(resolve,spec.durationSeconds*1000+300));recorder.stop();await new Promise(resolve=>recorder.onstop=resolve);clearInterval(ticker);const actualMime=recorder.mimeType||mime||'video/webm',blob=new Blob(chunks,{type:actualMime}),hash=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',await blob.arrayBuffer()))).map(byte=>byte.toString(16).padStart(2,'0')).join(''),url=URL.createObjectURL(blob),link=document.querySelector('#videoDownload'),extension=actualMime.startsWith('video/mp4')?'mp4':'webm',detail={durationSeconds:spec.durationSeconds,width:1080,height:1920,mimeType:actualMime,bytes:blob.size,sha256:hash,fileName:'jarvis-reel-'+spec.durationSeconds+'s.'+extension,container:extension,formatFallback:extension!=='mp4',qualityGatePassed,renderedFrameCount,averageRenderedFps:renderedFrameCount/spec.durationSeconds,...readiness,...audioRouting};window.__JARVIS_LAST_REEL_BLOB__=blob;window.__JARVIS_LAST_REEL_DETAIL__=detail;link.href=url;link.download=detail.fileName;link.hidden=false;window.dispatchEvent(new CustomEvent('jarvis:reel-exported',{detail}));statusEl.textContent=(extension==='mp4'?'MP4':'WebM fallback')+' verificado: '+blob.size+' bytes Â· SHA-256 '+hash.slice(0,12)+'â€¦';event.currentTarget.disabled=false;exporting=false;if(!window.__JARVIS_HEADLESS_EXPORT__)setTimeout(()=>link.click(),0)};draw(0);
...
export function describeReelStudio(input = {}, html = "") {
    return {
        ...
        checks: {
            ...
            deadlineCompensatedFramePacing: html.includes("function scheduleFrame") && html.includes("targetDelay=1000/30") && html.includes("Math.max(0,targetDelay-elapsed)"),
            exportCanvasOptimized: html.includes("desynchronized:true") && html.includes("headlineCache") && html.includes("activeSceneId")
        }
    };
}
