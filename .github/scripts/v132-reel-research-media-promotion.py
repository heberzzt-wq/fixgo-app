...
helper = r'''function verifiedResearchSourceUrls(
    ...
    const completedTasks =
        Array.isArray(missionState?.completedTasks)
    ...
    for (const task of completedTasks) {
        ...
        for (const source of task.observation.validSources) {
            ...
            try {
                ...
                // Ignore malformed research references.
            }
...
'''
new_selection = '''        const candidateAnchors =
            explicitAnchors.length > 0
                ? explicitAnchors
...
test("v132 promotes one verified researched URL into web.media.collect", () => {
    const calls = plannerTest.trustedPlanCalls({
        ...
        toolCalls: [{
            name: "web.media.collect",
            args: { requireImages: true, requireVideos: true }
        }]
    }, [mediaTool], {
...
test("v132 never promotes unverified or incomplete research tasks", () => {
    ...
    const promoted = plannerTest.verifiedResearchSourceUrls(missionState);
    assert.deepEqual(promoted, []);
});

test("v132 stays fail-closed when researched media sources are ambiguous", () => {
    const calls = plannerTest.trustedPlanCalls({
        planKind: "MISSION_CONTRACT_AUDIT",
        toolCalls: [{
            name: "web.media.collect",
        ...
        originalInstruction: "Investiga la entidad y crea un reel con medios reales.",
        missionState: completedResearch([
...
test("v132 explicit user source remains authoritative over researched URLs", () => {
    const calls = plannerTest.trustedPlanCalls({
        planKind: "MISSION_CONTRACT_AUDIT",
        toolCalls: [{
            name: "web.media.collect",
