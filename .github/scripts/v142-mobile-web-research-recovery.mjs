...
function testBlocks(source) {
  ...
  return {
    ...
    blocks: starts.map((start, i) => source.slice(start, i + 1 < starts.length ? starts[i + 1] : source.length))
  };
}
...
semantic = replaceSection(semantic, "async function runJarvisSemanticPlanner(", "async function runJarvisSemanticResponse(", plannerImpl, "authenticated-planner");
...
semantic = replaceSection(semantic, "async function runJarvisSemanticResponse(", "module.exports = {", responseImpl, "authenticated-response")
...
if (!chain.includes('startsWith("gemini-3.")')) {
...
if (!chain.includes("    responseHasExecutableSemanticPlan,")) chain = chain.replace("    resolveGroundingRedirectUrl,\n", "    resolveGroundingRedirectUrl,\n    responseHasExecutableSemanticPlan,\n");
write(P.chain, chain);
...
for (const block of parsed.blocks) {
  ...
  const legacyPublic =
    block.toLowerCase().includes("pollinations") ||
