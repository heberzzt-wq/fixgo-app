...
const PROPER_UI_LITERAL_PATTERN = /\\b(?:[A-ZÁÉÍÓÚÑ][A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9-]*[A-Z][A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9-]*|[A-ZÁÉÍÓÚÑ][a-záéíóúüñ0-9-]+(?:\\s+[A-ZÁÉÍÓÚÑ][A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9-]+)+)\\b/g;
...
if old_constants not in source:
    raise SystemExit("v4 constants anchor not found")
...
if anchor not in source:
    raise SystemExit("v4 helper insertion anchor not found")
...
if old_literal_line not in source:
    raise SystemExit("v4 literal extraction anchor not found")
...
if old_assert_end not in source:
    raise SystemExit("v4 recommendation insertion anchor not found")
...
if old_validate not in source:
    raise SystemExit("v4 validate anchor not found")
...
if old_policy not in source:
    raise SystemExit("v4 policy anchor not found")
...
if old_prompt not in source:
    raise SystemExit("v4 prompt anchor not found")
...
if old_repairable not in source:
    raise SystemExit("v4 repairable anchor not found")
