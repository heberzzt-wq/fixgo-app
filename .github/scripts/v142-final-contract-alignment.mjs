...
engine = replaceExactOnce(engine, '"test -x \\"$VENV/bin/python\\" || python3 -m venv \\"$VENV\\"",', '"test -x \\"$VENV/bin/python\\" || python3 -m venv --system-site-packages \\"$VENV\\"",', "V142_HUMO_SYSTEM_SITE_PACKAGES_VENV");
...
for (const marker of [NEW_HUMO_IMAGE,NEW_HUMO_DIGEST,'runtimePreflightCertified: false','physicalRuntimeCertified: false','physicalRuntimeCertification: null','python3 -m venv --system-site-packages',"importlib.metadata.version('torch').startswith('2.5.1')",'bootstrapDiagnostics = state.phase === "BOOTSTRAPPING"','await captureBootstrapFailureDiagnostics(state)']) {
  if (!engine.includes(marker)) throw new Error(`V142_HUMO_PREINSTALLED_RUNTIME_MISSING:${marker}`);
}
if (engine.includes('pip install torch==2.5.1 torchvision==0.20.1 torchaudio==2.5.1')) throw new Error("V142_HUMO_PAID_TORCH_INSTALL_REGRESSION");
