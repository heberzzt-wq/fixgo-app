...
function createRequestB2cWithdrawalHandler({ admin, db, functions, now = () => Date.now() }) {
    ...
    return async (data, context) => {
        ...
        if (typeof data?.amount !== "number" || !Number.isFinite(requestedAmount) ||
            requestedCents <= 0 || !Number.isSafeInteger(requestedCents)) {
        ...
        return db.runTransaction(async transaction => {
            ...
            if (!profileSnapshot.exists ||
                !platformContract.technicianEligibility(profile, { requireAvailable: false }).ok) {
            ...
            if (existingWithdrawal.exists) {
                ...
                if (existing.tecnico_id !== technicianId || Number(existing.monto) !== amount) {
            ...
            if (guardedWithdrawalId) {
                const guardedSnapshot = await transaction.get(db.collection("retiros").doc(guardedWithdrawalId));
            ...
            transaction.set(guardRef, {
                pending_withdrawal_id: withdrawalId,
                authority: "solicitarRetiro",
                updated_at: admin.firestore.FieldValue.serverTimestamp()
            }, { merge: true });

            return { ok: true, withdrawalId, amount, replay: false };
        });
    };
}
